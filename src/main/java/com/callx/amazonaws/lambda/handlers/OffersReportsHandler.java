	...
	public List<GeneralReportDTO> handleRequest(Request input, Context context) {
		...
		try {
			...
			if(conn != null) {
				...
				//String query = "select * from calls_reporting.callx_history_parquet limit 10";
				String query = AthenaQuerysList.OFFERS.replace("?1", String.valueOf(from));
				...
				if(results != null){
					System.out.println("Size of the OffersReportsHandler : "+results.size());
				}
			}
